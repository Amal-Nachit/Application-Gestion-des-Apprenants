...
final class Database
{
    ...
    if ($this->testIfTableFilmsExists()) {
      ...
      try {
        ...
        while ($migrationExistante === TRUE) {
          $migration = __DIR__ . "/../Migrations/gestionApprenants.sql";
  ...
  {
    $existant = $this->DB->query('SHOW TABLES FROM ' . DB_NAME . ' like \'%aga_user%\'')->fetch();

    if ($existant !== false && $existant[0] == "aga_user") {
      return true;
    } else {
      return false;
    }
